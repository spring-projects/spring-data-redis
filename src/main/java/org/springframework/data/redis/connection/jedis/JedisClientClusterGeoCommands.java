 ...
 */
@NullUnmarked
class JedisClientClusterGeoCommands implements RedisGeoCommands {

	private final JedisClientClusterConnection connection;

	JedisClientClusterGeoCommands(JedisClientClusterConnection connection) {
	...
	public Long geoAdd(byte @NonNull [] key, @NonNull Point point, byte @NonNull [] member) {
		...
		try {
			return connection.getClusterClient().geoadd(key, point.getX(), point.getY(), member);
		} catch (Exception ex) {
	...
	public Long geoAdd(byte @NonNull [] key, @NonNull Map<byte @NonNull [], @NonNull Point> memberCoordinateMap) {
		...
		try {
			return connection.getClusterClient().geoadd(key, redisGeoCoordinateMap);
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public Distance geoDist(byte @NonNull [] key, byte @NonNull [] member1, byte @NonNull [] member2) {
		...
		try {
			return JedisConverters.distanceConverterForMetric(DistanceUnit.METERS)
					.convert(connection.getClusterClient().geodist(key, member1, member2));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public Distance geoDist(byte @NonNull [] key, byte @NonNull [] member1, byte @NonNull [] member2,
		...
		try {
			return JedisConverters.distanceConverterForMetric(metric)
					.convert(connection.getClusterClient().geodist(key, member1, member2, geoUnit));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public List<@NonNull String> geoHash(byte @NonNull [] key, byte @NonNull [] @NonNull... members) {
		...
		try {
			return JedisConverters.toStrings(connection.getClusterClient().geohash(key, members));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public GeoResults<@NonNull GeoLocation<byte[]>> geoRadius(byte @NonNull [] key, @NonNull Circle within) {
		...
		try {
			return JedisConverters.geoRadiusResponseToGeoResultsConverter(within.getRadius().getMetric())
					.convert(connection.getClusterClient().georadius(key, within.getCenter().getX(), within.getCenter().getY(),
	...
	public GeoResults<@NonNull GeoLocation<byte[]>> geoRadiusByMember(byte @NonNull [] key, byte @NonNull [] member,
		...
		try {
			return JedisConverters.geoRadiusResponseToGeoResultsConverter(radius.getMetric())
					.convert(connection.getClusterClient().georadiusByMember(key, member, radius.getValue(), geoUnit));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public GeoResults<@NonNull GeoLocation<byte[]>> geoSearch(byte @NonNull [] key,
		...
		try {
			...
			return JedisConverters.geoRadiusResponseToGeoResultsConverter(predicate.getMetric())
					.convert(connection.getClusterClient().geosearch(key, params));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public Long geoSearchStore(byte @NonNull [] destKey, byte @NonNull [] key, @NonNull GeoReference<byte[]> reference,
		...
		try {
			...
			if (args.isStoreDistance()) {
				return connection.getClusterClient().geosearchStoreStoreDist(destKey, key, params);
			}

			return connection.getClusterClient().geosearchStore(destKey, key, params);
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
