 ...
 */
@NullUnmarked
class JedisClientClusterScriptingCommands implements RedisScriptingCommands {

	private final JedisClientClusterConnection connection;

	JedisClientClusterScriptingCommands(@NonNull JedisClientClusterConnection connection) {
	...
	public <T> T evalSha(byte @NonNull [] scriptSha, @NonNull ReturnType returnType, int numKeys,
		...
		try {
			return (T) new JedisScriptReturnConverter(returnType)
					.convert(connection.getClusterClient().evalsha(scriptSha, numKeys, keysAndArgs));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
