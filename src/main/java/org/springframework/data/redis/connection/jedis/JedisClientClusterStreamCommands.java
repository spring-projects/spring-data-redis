 ...
 * @since 4.1
 */
class JedisClientClusterStreamCommands implements RedisStreamCommands {

	private final JedisClientClusterConnection connection;

	JedisClientClusterStreamCommands(JedisClientClusterConnection connection) {
	...
	public Long xAck(byte[] key, String group, RecordId... recordIds) {
		...
		try {
			return connection.getClusterClient().xack(key, JedisConverters.toBytes(group),
	...
	public List<RecordId> xClaimJustId(byte[] key, String group, String newOwner, XClaimOptions options) {
		...
		try {
			...
			List<byte[]> ids = connection.getClusterClient().xclaimJustId(key, JedisConverters.toBytes(group),
	...
	public List<ByteRecord> xClaim(byte[] key, String group, String newOwner, XClaimOptions options) {
		...
		XClaimParams xClaimParams = StreamConverters.toXClaimParams(options);
		try {
			return convertToByteRecord(key, connection.getClusterClient().xclaim(key, JedisConverters.toBytes(group),
					JedisConverters.toBytes(newOwner), minIdleTime, xClaimParams, entryIdsToBytes(options.getIds())));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	...
	public Long xDel(byte[] key, RecordId... recordIds) {
		...
		try {
			return connection.getClusterClient().xdel(key, entryIdsToBytes(Arrays.asList(recordIds)));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public String xGroupCreate(byte[] key, String groupName, ReadOffset readOffset, boolean mkStream) {
		...
		try {
			return connection.getClusterClient().xgroupCreate(key, JedisConverters.toBytes(groupName),
	...
	public Boolean xGroupDelConsumer(byte[] key, Consumer consumer) {
		...
		try {
			return connection.getClusterClient().xgroupDelConsumer(key, JedisConverters.toBytes(consumer.getGroup()),
	...
	public Boolean xGroupDestroy(byte[] key, String groupName) {
		...
		try {
			return connection.getClusterClient().xgroupDestroy(key, JedisConverters.toBytes(groupName)) != 0L;
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public StreamInfo.XInfoStream xInfo(byte[] key) {
		...
		try {
			return StreamInfo.XInfoStream.fromList((List) connection.getClusterClient().xinfoStream(key));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public StreamInfo.XInfoGroups xInfoGroups(byte[] key) {
		...
		try {
			return StreamInfo.XInfoGroups.fromList(connection.getClusterClient().xinfoGroups(key));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public StreamInfo.XInfoConsumers xInfoConsumers(byte[] key, String groupName) {
		...
		try {
			return StreamInfo.XInfoConsumers.fromList(groupName,
					connection.getClusterClient().xinfoConsumers(key, JedisConverters.toBytes(groupName)));
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public Long xLen(byte[] key) {
		...
		try {
			return connection.getClusterClient().xlen(key);
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public PendingMessagesSummary xPending(byte[] key, String groupName) {
		...
		try {
			...
			Object response = connection.getClusterClient().xpending(key, group);
	...
	public PendingMessages xPending(byte[] key, String groupName, XPendingOptions options) {
		...
		try {
			...
			List<Object> response = connection.getClusterClient().xpending(key, group, pendingParams);
	...
	public List<ByteRecord> xRevRange(byte[] key, Range<String> range, Limit limit) {
		...
		try {
			return convertToByteRecord(key, connection.getClusterClient().xrevrange(key,
	...
	public Long xTrim(byte[] key, long count, boolean approximateTrimming) {
		...
		try {
			return connection.getClusterClient().xtrim(key, count, approximateTrimming);
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
	...
	public Long xTrim(byte[] key, XTrimOptions options) {
		...
		try {
			return connection.getClusterClient().xtrim(key, xTrimParams);
		} catch (Exception ex) {
			throw convertJedisAccessException(ex);
		}
	}
