...
When using Lettuce, the configuration above creates a connection to the current master and resolves the master again when the connection is lost. To refresh the topology from Sentinel events such as `+switch-master` while the existing connection remains open, configure Lettuce to use its Master/Replica connection mode:
...
This enables Lettuce to subscribe to Sentinel topology events and update the current master after a promotion. `ReadFrom` controls the connection mode and read routing; write commands are still sent to the current master. In Spring Boot, the equivalent property is `spring.data.redis.lettuce.read-from=upstream`.
