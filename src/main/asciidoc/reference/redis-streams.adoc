...
* Appending or sending messages
* Consuming or receiving messages
...
template.streamOps().add("my-stream", Collections.singletonMap("Hello", "World!"));
----

Stream messages carry a `Map`, key-value tuples, as their payload. Appending a message to a stream returns the message Id that can be used as further reference.

[[redis.streams.receive]]
== Consuming (Receiving Messages)

On the consuming side, one can consume one or multiple streams. Redis Streams provide read commands that allow consumption of the stream from an arbitrary position (random access) within the known stream content and beyond the stream end to consume new stream messages. 
...
To consume stream messages, one can either poll for messages in application code, or use one of the two <<redis.streams.receive.containers>>, the imperative or the reactive one. Each time a new message arrives, the container notifies the application code.
...
List<StreamMessage<K, V>> messages = template.streamOps().read(StreamReadOptions.empty().count(2),
				StreamOffset.create("my-stream", ReadOffset.latest()));

List<StreamMessage<K, V>> messages = template.streamOps().read(Consumer.from("my-group", "my-consumer"), 
...
* `StreamMessageListenerContainer` acts as message listener container for imperative programming models. It is used to consume messages from a Redis Stream and drive the `StreamListener` instances that are injected into it.
...
Subscription subscription = container.receive(StreamOffset.create("my-stream", ReadOffset.from("0-0")), streamListener);
----
