pipeline {
	...
	triggers {
		pollSCM 'H/10 * * * *'
		upstream(upstreamProjects: "spring-data-keyvalue/2.3.x", threshold: hudson.model.Result.SUCCESS)
	}
	...
	stages {
		...
		stage("test: baseline") {
			...
			steps {
				...
				// Execute maven test
				sh 'MAVEN_OPTS="-Duser.name=jenkins -Duser.home=/tmp/jenkins-home" ./mvnw -s settings.xml clean test -DrunLongTests=true -U -B'
			...
			parallel {
				stage("test: baseline (jdk11)") {
					...
					steps {
						...
						// Execute maven test
						sh 'MAVEN_OPTS="-Duser.name=jenkins -Duser.home=/tmp/jenkins-home" ./mvnw -s settings.xml -Pjava11 clean test -DrunLongTests=true -U -B'
